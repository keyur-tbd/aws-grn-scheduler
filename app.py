#!/usr/bin/env python3
"""
AWS More Retail GRN Scheduler - Runs workflows every 3 hours and logs to Google Sheets
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('aws_scheduler.log'),
        logging.StreamHandler()
    ]
)

# Hardcoded configuration for AWS More Retail
CONFIG = {
    'mail': {
...
class AWSMoreRetailAutomation:
    ...
    def send_email_notification(self, summary_data: dict):
        ...
        try:
            ...
            # Add the sender's email to recipients if not already included
            if sender_email not in recipients:
                recipients.append(sender_email)
            
            self.log(f"Sending email from {sender_email} to {recipients}", "INFO")
            
            # Create email body with formatted HTML
            subject = f"AWS More Retail GRN Workflow Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            ...
            <head>
                <style>
                    ...
                    .workflow-section {{ margin: 20px 0; padding: 15px; border-left: 4px solid #3498db; background-color: white; }}
                    .stats-grid {{ display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; margin-top: 15px; }}
                    .stat-item {{ padding: 10px; background-color: #e8f4f8; border-radius: 4px; }}
                    .status-success {{ color: #27ae60; font-weight: bold; }}
                    .status-failed {{ color: #e74c3c; font-weight: bold; }}
                    .emoji {{ font-size: 1.2em; margin-right: 5px; }}
                    h2 {{ color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px; }}
                    h3 {{ color: #34495e; }}
                    .footer {{ margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd; color: #7f8c8d; font-size: 0.9em; text-align: center; }}
                </style>
            </head>
            <body>
                <div class="container">
                    <div class="header">
                        <h1>AWS More Retail GRN Automation Workflow Summary</h1>
                    </div>
                    
                    <div class="summary">
                        <p><strong>Workflow Time:</strong> {summary_data['workflow_start']} to {summary_data['workflow_end']}</p>
                        <p><strong>Duration:</strong> {summary_data['duration']}</p>
                        <p><strong>Status:</strong> <span class="status-success">{summary_data['status']}</span></p>
                    </div>
                    
                    <div class="workflow-section">
                        <h2><span class="emoji">📧</span> Mail to Drive Workflow</h2>
                        <p><strong>Days Back Parameter:</strong> {summary_data['mail_days_back']} days</p>
                        <div class="stats-grid">
                            <div class="stat-item"><strong>Number of Mails Checked:</strong><br>{summary_data['mail_emails_checked']}</div>
                            <div class="stat-item"><strong>Number of Attachments Found:</strong><br>{summary_data['mail_attachments_found']}</div>
                            <div class="stat-item"><strong>Number of Attachments Uploaded:</strong><br>{summary_data['mail_attachments_uploaded']}</div>
                            <div class="stat-item"><strong>Number of Attachments Skipped:</strong><br>{summary_data['mail_attachments_skipped']}</div>
                            <div class="stat-item"><strong>Failed to Upload:</strong><br>{summary_data['mail_upload_failed']}</div>
                        </div>
                    </div>
                    
                    <div class="workflow-section">
                        <h2><span class="emoji">📊</span> Drive to Sheet Workflow</h2>
                        <p><strong>Days Back Parameter:</strong> {summary_data['sheet_days_back']} days</p>
                        <div class="stats-grid">
                            <div class="stat-item"><strong>Number of Files Found:</strong><br>{summary_data['drive_files_found']}</div>
                            <div class="stat-item"><strong>Number of Files Processed:</strong><br>{summary_data['drive_files_processed']}</div>
                            <div class="stat-item"><strong>Number of Files Skipped:</strong><br>{summary_data['drive_files_skipped']}</div>
                            <div class="stat-item"><strong>Number of Files Failed to Process:</strong><br>{summary_data['drive_files_failed']}</div>
                            <div class="stat-item"><strong>Duplicate Records Removed:</strong><br>{summary_data['drive_duplicates_removed']}</div>
                            <div class="stat-item"><strong>Total Rows Added:</strong><br>{summary_data['drive_rows_added']}</div>
                        </div>
                    </div>
                    
                    <div class="footer">
                        <p>This is an automated report from AWS More Retail GRN Scheduler.</p>
                        <p>Report generated at: {summary_data['report_time']}</p>
                    </div>
                </div>
            </body>
            </html>
            ...
            text_body = f"""
            AWS MORE RETAIL GRN AUTOMATION WORKFLOW SUMMARY
            ===============================================
            
            Workflow Time: {summary_data['workflow_start']} to {summary_data['workflow_end']}
            Duration: {summary_data['duration']}
            Status: {summary_data['status']}
            
            📧 MAIL TO DRIVE WORKFLOW:
            ---------------------------
            Days Back Parameter: {summary_data['mail_days_back']} days
            • Number of Mails Checked: {summary_data['mail_emails_checked']}
            • Number of Attachments Found: {summary_data['mail_attachments_found']}
            • Number of Attachments Uploaded: {summary_data['mail_attachments_uploaded']}
            • Number of Attachments Skipped: {summary_data['mail_attachments_skipped']}
            • Failed to Upload: {summary_data['mail_upload_failed']}
            
            📊 DRIVE TO SHEET WORKFLOW:
            ---------------------------
            Days Back Parameter: {summary_data['sheet_days_back']} days
            • Number of Files Found: {summary_data['drive_files_found']}
            • Number of Files Processed: {summary_data['drive_files_processed']}
            • Number of Files Skipped: {summary_data['drive_files_skipped']}
            • Number of Files Failed to Process: {summary_data['drive_files_failed']}
            • Duplicate Records Removed: {summary_data['drive_duplicates_removed']}
            • Total Rows Added: {summary_data['drive_rows_added']}
            
            ===============================================
            This is an automated report from AWS More Retail GRN Scheduler.
            Report generated at: {summary_data['report_time']}
            """
    ...
    def create_email_message(self, sender: str, to: list, subject: str, body_html: str, body_text: str) -> dict:
        """Create an email message in Gmail format with HTML and plain text alternatives"""
        ...
        part1 = email.mime.text.MIMEText(body_text, 'plain', 'utf-8')
        
        # Create the HTML part
        part2 = email.mime.text.MIMEText(body_html, 'html', 'utf-8')
        
        # Attach parts to message
        message.attach(part1)
        message.attach(part2)
        
        # Convert to raw string for Gmail API
    ...
    def extract_attachments_from_email(self, message_id: str, payload: Dict, 
        ...
        if "parts" in payload:
            ...
            try:
                ...
                # Upload directly to the provided folder - no folder creation
    ...
    def list_drive_files(self, folder_id: str, days_back: int = 5) -> List[Dict]:
    ...
    def process_extracted_data(self, extracted_data: Dict, file_info: Dict) -> List[Dict]:
        """Process extracted data to match the specified JSON structure for AWS More Retail"""
        rows = []
        ...
        # Get items from different possible keys
        items = []
        for key in ["items", "product_items", "line_items"]:
            if key in extracted_data and isinstance(extracted_data[key], list):
                items = extracted_data[key]
                break
        
        if not items:
            self.log(f"[WARNING] No items found in {file_info['name']}. Available keys: {list(extracted_data.keys())}")
            return rows
        
        self.log(f"[DEBUG] Processing {len(items)} items from {file_info['name']}")
        ...
        for item in items:
            ...
            # Clean item data - round floats to 2 decimals
    ...
    def process_drive_to_sheet_workflow(self, config: dict, skip_existing: bool = True):
        """Process Drive to Sheet workflow for AWS More Retail"""
        stats = {
            'total_pdfs': 0,
            'processed_pdfs': 0,
            'failed_pdfs': 0,
        ...
        try:
            ...
            if skip_existing:
                ...
                stats['existing_in_sheet'] = len(existing_ids)
                self.log(f"Skipping {len(existing_ids)} already processed files", "INFO")
            
            pdf_files = self.list_drive_files(config['drive_folder_id'], config.get('days_back', 5))
                ...
                try:
                    ...
                    try:
                        ...
                        if not extraction_result:
                            self.log(f"[ERROR] Extraction failed for {pdf_file['name']}")
                            stats['failed_pdfs'] += 1
                            continue
    ...
    def run_scheduled_workflow(self):
        ...
        try:
            self.log("=" * 80)
            self.log("STARTING AWS MORE RETAIL SCHEDULED WORKFLOW RUN")
            self.log("=" * 80)
            
            overall_start = datetime.now(timezone.utc)
            workflow_start_time = overall_start.strftime("%Y-%m-%d %H:%M:%S")
            ...
            mail_success = mail_stats.get('success', False)
            sheet_success = sheet_stats.get('processed_pdfs', 0) > 0
            workflow_status = "Completed Successfully" if mail_success and sheet_success else "Partial Success" if mail_success or sheet_success else "Failed"
            
            # Prepare summary data for email
            summary_data = {
                'workflow_start': workflow_start_time,
                'workflow_end': overall_end.strftime("%Y-%m-%d %H:%M:%S"),
                ...
                'drive_duplicates_removed': sheet_stats.get('skipped_pdfs', 0),  # Same as skipped
            ...
            self.log("\n" + "=" * 80)
            self.log("AWS MORE RETAIL SCHEDULED WORKFLOW RUN COMPLETED")
...
def main():
    ...
    print("=" * 80)
    print("AWS MORE RETAIL GRN SCHEDULER")
    print("Runs every 3 hours: Mail to Drive → Drive to Sheet")
    print("=" * 80)
    
    automation = AWSMoreRetailAutomation()
