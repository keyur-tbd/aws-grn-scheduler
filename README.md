...
| Stage | What happens |
|---|---|
| **1. Gmail -> Drive** | Searches mail from `aws-report@moreretail.in` matching `GRN`, looking back 10 days, and saves each attachment to a Drive folder. Already-seen files are skipped. |
| **2. Extract** | Each new file is parsed by **LlamaParse** using the LlamaCloud extract agent `More retail Agent`. |
| **3. Sheets** | Rows are appended to tab `mraws` of the tracking spreadsheet. |
| **4. Supabase** | `supabase_sink.py --run --source mraws` writes the same rows to table `mraws_grn`. Any field without a typed column is preserved in `raw_data` (jsonb), so a renamed or new field never fails a run. |
| **5. Run log** | A per-run summary is appended to tab `aws_workflow_logs`. |
...
The workflow deliberately does **not** call `main()`. `app.py` uses the `schedule` library for standalone local use, and that loop would sit idle burning runner minutes. Actions instead invokes `AWSMoreRetailAutomation.run_scheduled_workflow()` directly, then runs the Supabase sink as a separate step.

Recent runs average **~29 minutes**.
...
python supabase_sink.py --run --source mraws --limit 2
```
...
| File | Role |
|---|---|
| `app.py` | Gmail -> Drive -> extract -> Sheets, class `AWSMoreRetailAutomation` |
...
The extractor's output reaches Supabase whether or not a typed column exists. To promote a field: extend the `mraws` entry in `SOURCES` in `supabase_sink.py`, run `--print-schema`, and apply the `alter table` it prints. Until then, query it as `raw_data->>'your_key'`.
